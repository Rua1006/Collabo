...
import com.reser.service.MemberService;
...
public class MemberController {
	...
	public String getMember(@RequestParam String user_id, Model model) throws Exception {
	...
	public void idCheck(HttpServletResponse response, HttpServletRequest request, Model model) throws Exception {
		...
		dto = memberService.getMember(user_id);
	...
	public String memberWrite(MemberDTO member, Model model) throws Exception {
		//비밀번호 암호화
		String userpw = member.getUser_pw();
	...
	public String memberSignin(@RequestParam String user_id, @RequestParam String user_pw, HttpServletRequest req, RedirectAttributes rttr) throws Exception {
		session.invalidate();
		MemberDTO mdto = new MemberDTO();
		mdto.setUser_pw(user_pw);
		mdto.setUser_id(user_id);
		MemberDTO login = memberService.signIn(mdto);
		boolean loginSuccess = pwdEncoder.matches(mdto.getUser_pw(), login.getUser_pw());
	...
	public String memberLoginCtrl(MemberDTO mdto, RedirectAttributes rttr) throws Exception {
		...
		boolean mat = pwdEncoder.matches(mdto.getUser_pw(), member.getUser_pw());
	...
	public String memberUpdate(MemberDTO mdto, Model model) throws Exception {
		String pwd = pwdEncoder.encode(mdto.getUser_pw());
		mdto.setUser_pw(pwd);
	...
	public String memberDelete(@RequestParam String user_id, Model model, HttpSession session) throws Exception {
