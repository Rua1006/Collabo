...
<div>
	<table>
		...
		<c:forEach items="${reviewList }" var="review" varStatus="status">
			<tr>
				<td>${status.count }</td>
				<td><a href="${path1 }/review/detail.do?rno=${review.rno }">${review.title }</a></td>
				<td>
					<fmt:parseDate value="${review.regdate }" var="resdate" pattern="yyyy-MM-dd HH:mm:ss" />
	...
	<%-- <c:if test='${sid eq "admin"}'>  --%> 
		  ...
		  <a class="button" href="${path1 }/review/insert.do">글쓰기</a>
		</div>
	<%-- </c:if> --%>
</div>	
